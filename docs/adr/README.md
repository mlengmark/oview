...
| [0004](0004-what-non-windows-ci-could-and-could-not-prove.md) | What non-Windows CI could and could not prove for O-view | Proposed | A Linux CI job would run Core, Core.Tests, Linux and Linux.Tests, not the ADR-0003 anti-drift harness or the Tray skin, which are Windows-only. Recommends a two-job workflow (Windows: whole solution; Linux: the `net10.0` projects) and no harness restructuring. Awaiting a board yes/no. |
